...
from monitor.core import i18n
from monitor.core import logging as mylogger
from monitor.core import utils
from monitor.core import xmlutils
from monitor.middlewares import lazy_init
...
def initialize_db():
    ...
    CONF = config.CONF
    dbs = ["uic", "alarms", "graph", "dashboard", "capacity"]
    try:
    ...
    try:
        if CONF.dbs:
            ...
            param = db_confs.values()[1]
            default_database_name_index = param["connection"].rindex(r'/')
            default_connection = param["connection"][:default_database_name_index+1]
            conns = {}
            for name in dbs:
                param["connection"] = default_connection + name
...
def initialize_applications(api):
    """初始化wsgi application"""
    application_names = [
        "monitor.apps.host_manage",
        "monitor.apps.host_report",
        "monitor.apps.network_manage",
        "monitor.apps.db_instance",
        "monitor.apps.auth",
        "monitor.apps.monitor_env",
        "monitor.apps.alert",
        "monitor.apps.database_server",
        "monitor.apps.collect_cfg",
        "monitor.apps.audit",
        "monitor.apps.ceph_cluster",
        "monitor.apps.load_balance"
    ]
...
def initialize_middlewares(middlewares=None, override_defalut=False):
    """初始化中间件"""
    from monitor.middlewares import json_translator
    from monitor.middlewares import limiter
    from monitor.middlewares import globalvars
