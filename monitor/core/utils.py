...
def get_diff_time(time, now=None):
    """
    计算time与当前时间(now)的时间差，返回计算出最大单位时间
    :param time: <type 'datetime.datetime'>
    :param now: <type 'datetime.datetime'> 当前时间，或者传入的时间
    :return: 年 月 天 时 分 秒
    """
    if not now:
        now = datetime.datetime.now()
    if not isinstance(time, datetime.datetime) or not isinstance(now, datetime.datetime):
        return ""
    days = (now - time).days
    if days > 0:
        years = (days - 1) // 365
...
def decrypt(string, key="ABCD-DBACFFF-KEY"):
    """
    解密
    :param string:
    :param key:
    ...
    return k.decrypt(decodestring(string))
