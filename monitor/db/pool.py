...
from monitor.core import decorators as deco
...
class DBPool(object):
    ...
    def reflesh(self, param):
        ...
        connection = param_new.pop('connection')
        connection_passwd = param_new.pop('connection_passwd', '')
        connection = connection % decrypt(connection_passwd)
        self._pool = sessionmaker(bind=sqlalchemy.create_engine(connection, **param_new), autocommit=True)
        return True
