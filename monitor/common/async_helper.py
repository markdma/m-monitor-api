...
from monitor.common import celery
from monitor.core import config
from monitor.core import exceptions
from monitor.core import utils as talos_util
from monitor.utils import http
...
def send_callback(url_base, func, data, request_context=None, **kwargs):
    ...
    monitor>=1.3.0: 建议使用func.remote(data, xxx)方式进行更便捷的远程调用
    monitor>=1.3.0: 移除了原有的timeout参数，全部封装到request_context参数中，默认有timeout=10,verify=False
